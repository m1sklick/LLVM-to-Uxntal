//===-- UxnISelLowering.cpp - Uxn DAG Lowering Implementation ---------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the UxnTargetLowering class.
//
//===----------------------------------------------------------------------===//
...
const char *UxnTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  default:
    return NULL;
  case UxnISD::RET_FLAG: return "RetFlag";
  case UxnISD::LOAD_SYM: return "LOAD_SYM";
  case UxnISD::MOVEi32:  return "MOVEi32";
  case UxnISD::CALL:     return "CALL";
  }
}

UxnTargetLowering::UxnTargetLowering(UxnTargetMachine &UxnTM)
    : TargetLowering(UxnTM), Subtarget(*UxnTM.getSubtargetImpl()) {
  // Set up the register classes.
  addRegisterClass(MVT::i32, &Uxn::GRRegsRegClass);

  // Compute derived properties from the register classes
  computeRegisterProperties(Subtarget.getRegisterInfo());

  setStackPointerRegisterToSaveRestore(Uxn::SP);
...
SDValue UxnTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  default:
...
SDValue UxnTargetLowering::LowerGlobalAddress(SDValue Op, SelectionDAG& DAG) const
{
  EVT VT = Op.getValueType();
  GlobalAddressSDNode *GlobalAddr = cast<GlobalAddressSDNode>(Op.getNode());
  SDValue TargetAddr =
      DAG.getTargetGlobalAddress(GlobalAddr->getGlobal(), Op, MVT::i32);
  return DAG.getNode(UxnISD::LOAD_SYM, Op, VT, TargetAddr);
}
...
#include "UxnGenCallingConv.inc"

//===----------------------------------------------------------------------===//
//                  Call Calling Convention Implementation
//===----------------------------------------------------------------------===//

/// Uxn call implementation
SDValue UxnTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(Outs, CC_Uxn);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    SDValue StackPtr = DAG.getRegister(Uxn::SP, MVT::i32);
  ...
  Chain = DAG.getNode(UxnISD::CALL, Loc, NodeTys, Ops);
  InFlag = Chain.getValue(1);
...
SDValue UxnTargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, RetCC_Uxn);
...
/// Uxn formal arguments implementation
SDValue UxnTargetLowering::LowerFormalArguments(
  ...
  CCInfo.AnalyzeFormalArguments(Ins, CC_Uxn);
  ...
  for (auto &VA : ArgLocs) {
    if (VA.isRegLoc()) {
      ...
      const unsigned VReg = RegInfo.createVirtualRegister(&Uxn::GRRegsRegClass);
...
bool UxnTargetLowering::CanLowerReturn(
    CallingConv::ID CallConv, MachineFunction &MF, bool isVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs, LLVMContext &Context) const {
  SmallVector<CCValAssign, 16> RVLocs;
  CCState CCInfo(CallConv, isVarArg, MF, RVLocs, Context);
  if (!CCInfo.CheckReturn(Outs, RetCC_Uxn)) {
    return false;
  }
...
SDValue
UxnTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool isVarArg,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_Uxn);
  ...
  return DAG.getNode(UxnISD::RET_FLAG, dl, MVT::Other, RetOps);
}
